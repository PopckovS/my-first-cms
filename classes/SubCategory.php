...
class SubCategory
{
    ...
    * @return subCategories|false Объект подкатегории или false, если запи  сь не найдена или возникли проблемы
    */
    ...
    * @param int $categoryId Вернуть подкатегорию только из категории с указанным ID
    ...
    */
   public static function getAllSubCategory($numRows = 1000000, $showParent = false) 
    {
        $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);

        $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM subCategory LIMIT :numRows";
       ...
       //trace($st);die();
        ...
        while ($row = $st->fetch()) 
        {
            $sub = new SubCategory($row);	
            $subCategory[] = $sub;
        }
        
        
            
        /* Если указан параметр $showParent = true то результар-щий массив будет суказанием
        родительской категории в виде Слов а не цифр */
        if($showParent)
        {
       ...
       //trace($list);die();
        ...
        return ( ["results" => $subCategory, "totalRows" => $totalRows[0]] );
    }
